  ...
  try {
    ...
    bot.start(async (ctx) => {
      try {
        ...
        console.log("conect");
        await ctx.reply(
          "Здравствуйте!\nЗдесь вы можете забронировать места или отменить бронирование на мероприятия в Stand Up Comedy Hall.\n\nДля этого ответьте, пожалуйста, на вопросы бота.\n\nВНИМАНИЕ! ВАЖНАЯ ИНФОРМАЦИЯ!\n❗️ Отчётные концерты (пятница, суббота) начинаются в 20:00 ❗️ Брони принимаются до 19.00 дня мероприятия.\n❗️❗️Оплата брони происходит на месте (картой или наличными) по факту прихода.\n❗️ Обращаем Ваше внимание, что Вы бронируете места за столиком либо за барной стойкой.\n❗️ Места за столиком рассчитаны на 4-х человек, поэтому просим Вас дружелюбно отнестись к возможной подсадке и гостям рядом с Вами. Надеемся на Ваше понимание и позитивное настроение.\n❗️ Для Вашего комфорта просим приходить МИНИМУМ за 30 минут до начала шоу (начало всех шоу, кроме Отчетного концерта в 20:00, Отчетный концерт в 21:00) это позволит Вам комфортно расположиться, а также сделать заказ и пообщаться с нашими барменами🍹\n\nС мероприятиями клуба можно ознакомиться:\nhttps://www.instagram.com/standupcomedyhall/",
          Markup.keyboard([["Выбрать шоу", "Отменить бронирование", "Сброс"]])
            .oneTime()
            .resize()
        );
